...
Esta guía te ayudará a desplegar tu aplicación y APIs en DigitalOcean usando Terraform y Ansible de manera sencilla.
...
Internet
    ↓
[App Platform] → [Load Balancer] → [Droplet 1] → [API 1] → [MySQL]
                                 → [Droplet 2] → [API 2] → [Bucket]
```
...
### 5. Imágenes Docker
Asegúrate de que tus imágenes estén en DockerHub:
- `tu-usuario/app:latest` - Tu aplicación principal
- `tu-usuario/api1:latest` - API que se conecta a MySQL
- `tu-usuario/api2:latest` - API que se conecta al Bucket
...
1. **Configura las variables en `terraform.tfvars`:**
```hcl
do_token = "dop_v1_tu_token_real_aqui"
ssh_key_name = "nombre-de-tu-clave-ssh"
docker_api1_image = "tu-usuario/api1:latest"
docker_api2_image = "tu-usuario/api2:latest"
app_docker_image = "tu-usuario/app:latest"
```
